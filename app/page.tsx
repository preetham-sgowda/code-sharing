...
export default function Page() {
  ...
  async function submitAuth(event: React.FormEvent) {
    ...
    if (!supabase) return
    const result = authMode === 'sign-in' ? await supabase.auth.signInWithPassword({ email, password }) : await supabase.auth.signUp({ email, password, options: { emailRedirectTo: process.env.NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL ?? `${window.location.origin}/auth/callback` } })
  ...
  return <main className="site-shell">
    <header className="topbar"><Logo /><nav className="nav-links"><a href="#explore">Explore</a><a href="#how">How it works</a><a href="#about">About</a></nav><div className="header-actions"><a className="github-link" href="https://github.com" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={14} /></a>{user ? <button className="avatar" onClick={() => supabase?.auth.signOut().then(() => setUser(null))}>{user.email?.slice(0, 1).toUpperCase()}</button> : <button className="sign-in" onClick={() => { setAuthMode('sign-in'); setAuthOpen(true) }}>Sign in <ArrowUpRight size={15} /></button>}<button className="mobile-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu"><Menu size={20} /></button></div></header>
